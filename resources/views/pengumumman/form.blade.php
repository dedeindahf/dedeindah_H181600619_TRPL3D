...
<div class="form-group row">
       ...
       <div class="form-group row">
        <label for="kategori_pengumumman_id" class="col-md-2 col-form-label text-md-right">{{ __('Kategori Pengumumman') }}</label>
            
            <div class="col-md-10">
                 {!! Form::select('kategori_pengumumman_id',$kategoriPengumumman, null,["class"=>"form-control","required"]) !!}

                  @error('kategori_pengumumman_id')
                  <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                  </span>
                  @enderror
   ...
   <input id="users_id" type="hidden" class="form-control @error('users_id') is-invalid @enderror" name="users_id" value="{{ Auth::id ()}}" required>
     ...
     <div class="form-group row mb-0">
          <div class="col-md-10 offset-md-2">
            ...
            <a href="{!! route('pengumumman.index') !!}" class="btn btn-danger">
                {{ __('batal') }}
            </a>
             </div>
        </div>